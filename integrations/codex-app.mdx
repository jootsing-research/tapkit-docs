---
title: Codex (App)
description: Control iPhones from the Codex desktop app
---

The TapKit plugin for Codex gives the agent iPhone control in the desktop app. It installs an MCP server connection and a skill file that teaches Codex how to use the tools effectively.

<Note>
  Codex can install and track the TapKit plugin marketplace from GitHub. You do not need to clone the plugin repo or create a marketplace file yourself.
</Note>

## Step 1: Install the Codex CLI

The Codex app uses the CLI to add and update plugin marketplaces. If you do not already have it, install the Codex CLI:

```bash
npm i -g @openai/codex
codex
```

The first time you run `codex`, sign in with your ChatGPT account or API key. See OpenAI's [Codex CLI setup guide](https://developers.openai.com/codex/cli) for more install options.

## Step 2: Add the TapKit marketplace

Run this command in your terminal:

```bash
codex plugin marketplace add jootsing-research/tapkit-plugins-codex
```

If the Codex app is already open, restart it so the new marketplace appears.

## Step 3: Open Plugins

In the Codex app, click **Plugins** in the left sidebar.

<Accordion title="See screenshot">
  <Frame>
    <div style={{ display: 'flex', justifyContent: 'center', padding: '16px' }}>
      <img src="/images/codex-app-plugins.png" alt="Codex app sidebar with Plugins selected" style={{ maxWidth: '70%', borderRadius: '12px' }} />
    </div>
  </Frame>
</Accordion>

## Step 4: Select the TapKit marketplace

On the Plugins page, open the marketplace dropdown and select **Tap Kit Plugins**.

<Accordion title="See screenshot">
  <Frame>
    <div style={{ display: 'flex', justifyContent: 'center', padding: '16px' }}>
      <img src="/images/codex-app-marketplace-dropdown.png" alt="Codex plugins page marketplace dropdown showing Tap Kit Plugins" style={{ maxWidth: '100%', borderRadius: '12px' }} />
    </div>
  </Frame>
</Accordion>

## Step 5: Install the TapKit plugin

Find **Tap Kit** under **Productivity**, click the **+** button, then click **Install Tap Kit** in the popup.

<Accordion title="See screenshots">
  <Frame>
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center', padding: '16px' }}>
      <img src="/images/codex-app-tapkit-not-installed.png" alt="Codex plugins page showing Tap Kit with plus button" style={{ maxWidth: '100%', borderRadius: '12px' }} />
      <img src="/images/codex-app-install-popup.png" alt="Install Tap Kit popup with Install Tap Kit button" style={{ maxWidth: '80%', borderRadius: '12px' }} />
    </div>
  </Frame>
</Accordion>

## Step 6: Confirm TapKit is installed

After installation, TapKit shows a checkmark in the plugin list.

<Accordion title="See screenshot">
  <Frame>
    <div style={{ display: 'flex', justifyContent: 'center', padding: '16px' }}>
      <img src="/images/codex-app-installed.png" alt="Codex plugins page showing Tap Kit installed with checkmark" style={{ maxWidth: '100%', borderRadius: '12px' }} />
    </div>
  </Frame>
</Accordion>

## Step 7: Authenticate

Once the plugin is loaded, the TapKit MCP server will prompt you to sign in via your browser.

## Verify it works

```
Take a screenshot of my phone
```

If a phone is connected via the Mac app, you'll see the screenshot.

## Upgrade

Refresh the TapKit marketplace when a new plugin version is available:

```bash
codex plugin marketplace upgrade tapkit-plugins
```

`tapkit-plugins` is the Codex marketplace name from the marketplace metadata.

## What's in the plugin

The TapKit plugin bundles:

- **MCP server config** — connects Codex to TapKit's hosted MCP endpoint
- **Skills** — teaches Codex how to use the coordinate system, screenshot loop, and gesture types

## Links

- [TapKit Codex plugin on GitHub](https://github.com/Jootsing-Research/tapkit-plugins-codex)
- [Codex plugin docs](https://developers.openai.com/codex/plugins)
- [API Reference](/api-reference/introduction)
